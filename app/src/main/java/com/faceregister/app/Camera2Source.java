...
import com.faceregister.app.utils.Utils;
...
public class Camera2Source
{
    ...
    private IOverlayView iOverlayViewCallback;
                ...
                if (faces.length > 0)
                {
                    for(int i = 0; i < faces.length; i++)
                    {
                        if (faces[i].getScore() > 50) {
                            ...
                            Log.i(TAG, "ACTIVITY RECT" + i + " bounds: " + uRect);

                            final Rect rect = uRect;

                            if(iOverlayViewCallback != null)
                            {
                                iOverlayViewCallback.setRect(rect);
                            }
     ...
     */
    public void setOverlayCallback(IOverlayView viewCallback)
    {
        iOverlayViewCallback = viewCallback;
    }
            ...
            mCameraDevice.createCaptureSession(Arrays.asList(surface, mImageReaderPreview.getSurface(), mImageReaderStill.getSurface()), new CameraCaptureSession.StateCallback() {
                ...
                public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession)
                {
                    ...
                    try
                    {
                        ...
                        mPreviewRequestBuilder.set(/*CaptureRequest.CONTROL_AF_MODE*/ CaptureRequest.STATISTICS_FACE_DETECT_MODE,
                                mFocusMode);
    ...
    public void setLockFocus(boolean lock)
    {
        lockFocus = lock;
    }
}