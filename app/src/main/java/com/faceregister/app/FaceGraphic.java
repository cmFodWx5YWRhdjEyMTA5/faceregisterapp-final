...
public class FaceGraphic extends GraphicOverlay.Graphic {
    ...
    public void draw(Canvas canvas) {
        ...
        //      IMP canvas.drawCircle(x, y - 4 * ID_Y_OFFSET, FACE_POSITION_RADIUS, facePositionPaint);
        //      IMP   canvas.drawText("id: " + face.getTrackingId(), x + ID_X_OFFSET, y - 3 * ID_Y_OFFSET, idPaint);
        //      IMP canvas.drawText("happiness: " + String.format("%.2f", face.getSmilingProbability()), x + ID_X_OFFSET * 3, y - 2 * ID_Y_OFFSET, idPaint);
        if (facing == CameraSource.CAMERA_FACING_FRONT) {
            //      IMP   canvas.drawText("right eye: " + String.format("%.2f", face.getRightEyeOpenProbability()), x - ID_X_OFFSET, y, idPaint);
            //      IMP  canvas.drawText("left eye: " + String.format("%.2f", face.getLeftEyeOpenProbability()), x + ID_X_OFFSET * 6, y, idPaint);
        } else {
            //      IMP canvas.drawText("left eye: " + String.format("%.2f", face.getLeftEyeOpenProbability()), x - ID_X_OFFSET, y, idPaint);
            //      IMP canvas.drawText("right eye: " + String.format("%.2f", face.getRightEyeOpenProbability()), x + ID_X_OFFSET * 6, y, idPaint);
        }
...
//      IMP  canvas.drawRect(left, top, right, bottom, boxPaint);
    ...
    private void drawLandmarkPosition(Canvas canvas, FirebaseVisionFace face, int landmarkID) {
        ...
        if (landmark != null) {
            ...
            //      IMP    canvas.drawCircle(translateX(point.getX()), translateY(point.getY()), 10f, idPaint);
        }
    }
    ...
    private void drawBitmapOverLandmarkPosition(Canvas canvas, FirebaseVisionFace face, int landmarkID) {
        ...
        if (overlayBitmap != null) {
            ...
            //      IMP   canvas.drawBitmap(overlayBitmap, null,new Rect(left, top, right, bottom), null);
        }
